        ...
        for (const row of tables) {
            ...
            `;
                for (const col of cols) {
                    ...
                    SELECT ${sql(colName)} FROM ${sql(tableName)} WHERE ${sql(colName)} ILIKE '%HUDSON%' LIMIT 1
                `;
                    if (matches.length > 0) {
                        console.log(`FOUND 'HUDSON' in table ${tableName}, column ${colName}:`, matches[0][colName]);
                    }
                }
            } catch (e) { }
        }
