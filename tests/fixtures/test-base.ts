...
export const test = base.extend<MyFixtures>({
  ...
  dbCleanup: [
    ...
    async ({}, use) => {
      ...
      const isProduction =
        supabaseUrl.includes('xexkttehzpxtviebglei') ||
      ...
      try {
        ...
        await supabase
          ...
          .delete()
          .or(
            'sku.ilike.TEST-%,sku.ilike.GHOST-%,sku.ilike.FIX-%,sku.ilike.DURABLE-%,sku.ilike.SKU-ADD%,sku.ilike.STAFF-%,sku.ilike.UNDO-RPC-%,sku.ilike.LIFO-%'
          );
        await supabase
          .from('inventory')
          .delete()
