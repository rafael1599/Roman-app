...
test.describe('Mutation Durability & Sync Lifecycle', () => {
  ...
  test('should persist through reload while offline and sync when online', async ({
    page,
    inventoryPage,
  }) => {
    ...
    await page.evaluate(() => {
      ...
      cache.forEach((m: any, i: number) => {
        console.log(
          `[MutationRegistry][TEST_INSPECT] #${i}: key=${JSON.stringify(m.options.mutationKey)} status=${m.state.status} isPaused=${m.state.isPaused}`
        );
        console.log(
          `[MutationRegistry][TEST_INSPECT] #${i} variables:`,
    ...
    await expect(page.locator('div.group.relative', { hasText: SKU }).first()).toBeVisible({
      timeout: 15000,
    });
