...
test.describe('Distribution Auto-Adjustment on Deduction', () => {
   ...
   */
  test('should adjust distribution when quantity is deducted via RPC', async ({
    supabaseAdmin,
  }) => {
    ...
    const totalAfter = dist.reduce((sum: number, d: DistEntry) => sum + d.count * d.units_each, 0);
    expect(totalAfter).toBe(65);

    // Verify priority order was respected: PALLETs consumed first
    const pallets = dist.filter((d: DistEntry) => d.type === 'PALLET');
    const palletTotal = pallets.reduce((s: number, d: DistEntry) => s + d.count * d.units_each, 0);
    expect(palletTotal).toBe(25); // residual from broken pallet

    const lines = dist.filter((d: DistEntry) => d.type === 'LINE');
    const lineTotal = lines.reduce((s: number, d: DistEntry) => s + d.count * d.units_each, 0);
    expect(lineTotal).toBe(15); // untouched (3×5)

    const towers = dist.filter((d: DistEntry) => d.type === 'TOWER');
    const towerTotal = towers.reduce((s: number, d: DistEntry) => s + d.count * d.units_each, 0);
   ...
   */
  test('should break a group and create residual when partially consumed', async ({
    supabaseAdmin,
  }) => {
    ...
    const { error: insertErr } = await supabaseAdmin.from('inventory').insert({
      sku,
      warehouse,
      location,
      quantity: 30,
      is_active: true,
      distribution: [{ type: 'PALLET', count: 1, units_each: 30 }],
    });
   ...
   */
  test('should consume types in priority order: PALLET first, then LINE, etc.', async ({
    supabaseAdmin,
  }) => {
   ...
   */
  test('should handle deduction exceeding distribution total gracefully', async ({
    supabaseAdmin,
  }) => {
  ...
  test.skip('should show updated distribution in modal after quantity edit', async ({
    ...
    const totalDist = dist.reduce((sum: number, d: DistEntry) => sum + d.count * d.units_each, 0);
