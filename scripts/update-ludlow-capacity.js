...
async function updateLudlowCapacities() {
  ...
  try {
    ...
    for (const location of uniqueLocations) {
      ...
      if (rowNum !== undefined) {
        ...
        if (newCapacity !== undefined) {
          console.log(
            `Updating capacity for location "${location}" (Row ${rowNum}) to ${newCapacity}`
          );
          ...
          console.log(
            `No new capacity data found for Row ${rowNum} (Location: ${location}). Skipping.`
          );
        }
      } else {
    ...
    console.log(
      `✅ Finished updating. Successfully updated ${updatedCount} unique LUDLOW locations.`
    );
  } catch (err) {
