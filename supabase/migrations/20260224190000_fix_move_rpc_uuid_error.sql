-- Migration: Fix UUID Error in move_inventory_stock and Column Name Mismatch
-- 1. Fix v_src_id type from UUID to BIGINT (matches inventory table id)
-- 2. Fix SELECT name to SELECT location (locations table column name)
-- 3. Ensure p_user_id null handling
...
CREATE OR REPLACE FUNCTION public.move_inventory_stock(
  ...
  p_qty INTEGER,
  p_performed_by TEXT,
  p_user_id UUID DEFAULT NULL, -- Added default NULL for safety
...
AS $$
DECLARE
  v_src_id BIGINT; -- Fixed: Was UUID, causing "invalid input syntax" when selecting bigint ID
...
BEGIN
  ...
  -- Fixed: Column name is 'location', not 'name'
