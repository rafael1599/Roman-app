-- Fix NULL location matching in adjust_inventory_quantity and move_inventory_stock
-- The previous resilience patch used simple '=' which fails for NULL locations (PDF imports)
...
BEGIN
  ...
  SELECT location INTO v_from_loc_name FROM locations WHERE id = v_from_loc_id;
  ...
  WHERE sku = p_sku AND warehouse = p_from_warehouse 
    AND UPPER(TRIM(COALESCE(location, ''))) = UPPER(TRIM(COALESCE(v_from_loc_name, '')))
  FOR UPDATE;
  ...
  PERFORM public.upsert_inventory_log(
    p_sku, p_from_warehouse, v_from_loc_name, p_to_warehouse, p_to_location,
