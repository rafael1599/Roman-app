...
-- ============================================================================
-- FINAL ATOMIC INVENTORY RPC SUITE (Option A + Auto-Deactivate)
-- ============================================================================
-- Purpose: 
--   1. Replace client-side calculations with server-side atomic operations.
--   2. Implement soft-delete pattern via is_active (boolean).
--   3. Auto-deactivate SKUs when quantity reaches 0.
-- ============================================================================
-- ----------------------------------------------------------------------------
-- STEP 1: SCHEMA MIGRATION (Execute this if not already done)
-- ----------------------------------------------------------------------------
-- ALTER TABLE inventory ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true;
-- UPDATE inventory SET is_active = (status = 'active') WHERE is_active IS NULL;
-- ALTER TABLE inventory DROP COLUMN IF EXISTS status;
-- ----------------------------------------------------------------------------
-- HELPER: Resolve or Create Location
-- ----------------------------------------------------------------------------
