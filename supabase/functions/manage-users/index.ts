...
serve(async (req) => {
    ...
    try {
        ...
        if (body.action === 'ping') {
            return new Response(JSON.stringify({ status: 'Connected', message: 'Function is reachable!' }), {
        ...
        if (authError || !user) {
            return new Response(JSON.stringify({ error: 'FUNC_ERROR: Invalid or Expired Token', details: authError }), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
                status: 401
            })
        }
        ...
        if (action === 'createUser') {
            ...
            if (typeof is_active === 'boolean') {
                // If inactive, we could also ban the user in Auth for extra security
