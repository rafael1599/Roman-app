...
export const SplitOrderModal: React.FC<SplitOrderModalProps> = ({ order, onClose, onSplitComplete }) => {
    ...
    const handleSplit = async () => {
        ...
        try {
            ...
            for (const source of sourceOrders) {
                ...
                const items = (groupedItems[orderNum] || []).map((item: any) => {
                    const { source_order, ...rest } = item;
                    return rest;
                });
            ...
            <div className="bg-card border border-subtle rounded-3xl w-full max-w-lg max-h-[80vh] overflow-y-auto shadow-2xl">
                ...
                <div className="flex items-center justify-between p-5 border-b border-subtle">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-black uppercase tracking-widest text-content">Split Combined Order</h3>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-surface transition-all">
                ...
                <div className="p-5 space-y-4">
                        ...
                        return (
                            <div key={i} className="p-4 bg-surface border border-subtle rounded-2xl space-y-2">
                                <div className="flex items-center justify-between">
                                    <span className="font-mono font-black text-lg text-content">#{source.order_number}</span>
                                ...
                                <div className="flex flex-wrap gap-1">
                                    {items.map((item: any, j: number) => (
                                        <span key={j} className="text-[10px] font-mono bg-main px-2 py-0.5 rounded-lg text-muted border border-subtle">
                    ...
                    {groupedItems['unknown'] && groupedItems['unknown'].length > 0 && (
                        <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl space-y-2">
                            <span className="text-xs font-bold text-amber-400">Untagged items (will stay in first order)</span>
                            <div className="flex flex-wrap gap-1">
                                {groupedItems['unknown'].map((item: any, j: number) => (
                                    <span key={j} className="text-[10px] font-mono bg-main px-2 py-0.5 rounded-lg text-muted">
                                        {item.sku} x{item.pickingQty}
                                    </span>
                                ))}
                            </div>
