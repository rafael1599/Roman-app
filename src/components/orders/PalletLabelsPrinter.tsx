    ...
    const generateAndPrintPDF = async () => {
        ...
        try {
            ...
            if (!isHistoryMode && customer?.id) {
                const needsUpdate = street !== customer.street || city !== customer.city || state !== customer.state || zip !== customer.zip_code;
                if (needsUpdate) {
                    ...
                    <div className="space-y-4 text-left">
                        ...
                        <div className="space-y-3 p-4 bg-zinc-800/30 rounded-2xl border border-zinc-700/50">
                            <div className="flex items-center justify-between mb-1">
                                <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">Destination</p>
                                <p className="text-accent text-[8px] font-black uppercase px-2 py-0.5 bg-accent/10 rounded-full border border-accent/10">
                                    {isHistoryMode ? 'Read Only' : (hasAddress ? 'Verified Address' : 'Missing Details')}
                                </p>
                            </div>

                            <p className="text-white font-bold text-sm mb-3 px-1">{customer?.name || 'GENERIC CUSTOMER'}</p>
