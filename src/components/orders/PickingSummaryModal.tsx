                ...
                <div className="px-8 py-7 border-b border-white/10 flex items-center justify-between shrink-0 bg-white/[0.02]">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <h3 className="text-white/30 text-[10px] font-black uppercase tracking-[0.2em]">Live Picking Summary</h3>
                        </div>
                        <h2 className="text-white text-3xl font-black tracking-tight leading-none">Order #{orderNumber}</h2>
                        {completedAt && (
                ...
                <div className="flex-1 overflow-y-auto p-8 no-scrollbar scroll-smooth">
                    {pallets.length === 0 ? (
                        <div className="py-32 flex flex-col items-center justify-center text-center">
                            ...
                            <p className="text-white/20 font-black uppercase tracking-widest text-xs">No picking items found for this order</p>
                        </div>
                    ) : (
                        pallets.map((pallet) => (
                                    ...
                                    {pallet.items.map((item: any, idx: number) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-col items-end">
                                                <span className="text-[9px] text-white/20 font-black uppercase tracking-[0.2em] mb-1">Row</span>
                ...
                <div className="px-8 py-6 bg-white/[0.03] border-t border-white/10 flex items-center justify-between shrink-0">
                    <div className="flex gap-10">
                        <div className="flex flex-col">
                            <span className="text-[9px] text-white/25 font-black uppercase tracking-[0.2em] mb-1">Pallet Count</span>
                            <span className="text-white text-xl font-black leading-none">{pallets.length}</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-[9px] text-white/25 font-black uppercase tracking-[0.2em] mb-1">Total Items</span>
                            <span className="text-white text-xl font-black leading-none">{totalUnits}</span>
                        </div>
                    </div>

                    <div className="flex flex-col items-end opacity-40">
                        <span className="text-[8px] text-white font-black uppercase tracking-widest mb-1">Status</span>
                        <div className="flex items-center gap-1.5">
                            <div className="w-1 h-1 rounded-full bg-emerald-500" />
                            <span className="text-[10px] text-white font-bold uppercase tracking-widest italic">Inventory Linked</span>
                        </div>
                    </div>
                </div>
            </div>
