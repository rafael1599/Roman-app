...
export const SlideToConfirm: React.FC<SlideToConfirmProps> = ({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- handleMove/handleEnd are intentionally excluded; they use refs and state that is already covered by isDragging and sliderWidth
  }, [isDragging, sliderWidth]);

  return (
    <div
      ref={containerRef}
      className={`relative h-12 rounded-full overflow-hidden select-none transition-all touch-none ${
    ...
    >
      {/* Background Text */}
      <div
        className={`absolute inset-0 flex items-center justify-center text-sm font-black uppercase tracking-widest transition-opacity duration-300 pointer-events-none ${
          isDragging || isConfirmed ? 'opacity-0' : 'opacity-40 text-muted'
        }`}
      >
        {text}
      </div>
      ...
      {/* Confirmed Text */}
      <div
        className={`absolute inset-0 flex items-center justify-center text-sm font-black uppercase tracking-widest text-white transition-opacity duration-300 pointer-events-none ${
      ...
      {!isConfirmed && (
        <div
          className={`absolute left-0 top-0 bottom-0 transition-all duration-75 ease-out ${
            variant === 'info' ? 'bg-blue-500/20' : 'bg-accent/20'
          }`}
          style={{
      ...
      {/* Thumb */}
      <div
        className={`absolute top-2 bottom-2 aspect-square rounded-full flex items-center justify-center shadow-lg transform transition-transform duration-75 ease-out z-10 ${
          isConfirmed
