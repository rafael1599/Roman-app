...
export const usePickingActions = ({
        ...
        try {
            ...
            for (const myItem of finalItems) {
                ...
                if (myQty > availableForMe) {
                    toast.error(`Stock conflict! ${myItem.SKU}: Only ${availableForMe} available (you need ${myQty}). Another user may have taken it.`, { duration: 5000 });
                    return null; // Abort
                }
            }
            // --- End Validation ---
    ...
    const deleteList = useCallback(async (listId: string | null, keepLocalState = false) => {
        ...
        try {
            ...
            for (const myItem of cartItems) {
                ...
                if (myQty > availableAcrossSystem) {
                    toast.error(`Constraint Error: ${myItem.SKU} - Only ${availableAcrossSystem} available.`, { duration: 5000 });
                    return; // Abort
                }
            }
