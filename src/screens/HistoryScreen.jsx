...
export const HistoryScreen = () => {
    ...
    const generateDailyPDF = useCallback((jsPDF, autoTable) => {
        ...
        doc.text(`Date: ${todayStr} | Time: ${new Date().toLocaleTimeString()}`, 0.2, 0.58);
        ...
        doc.setFontSize(9);
        doc.setFont(undefined, 'bold');
        doc.text(`Total SKU's: ${stats.total} | Total Qty: ${stats.qty}`, 4.0, 0.4, { align: 'right' });
        ...
        const tableData = dataToExport.map(log => {
            ...
            const whInfo = (log.from_warehouse && log.to_warehouse && log.from_warehouse !== log.to_warehouse)
                ? ` [${log.from_warehouse}→${log.to_warehouse}]`
                : (log.from_warehouse ? ` [${log.from_warehouse}]` : '');
        ...
        return doc;
    }, [filteredLogs, filter, userFilter, searchQuery, profile, authUser]);
    ...
    const handleDownloadReport = useCallback(async () => {
        try {
            ...
            doc.save(`${filterLabel}-report-${dateStr}.pdf`);
        } catch (err) {
