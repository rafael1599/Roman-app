...
export const InventoryScreen = () => {
  ...
  const handleDownloadView = useCallback(async () => {
    ...
    try {
      ...
      doc.setFontSize(36);
      doc.text('Stock View Report', 0.5, 0.8);

      doc.setFont('times', 'normal');
      doc.setFontSize(21);

      const headerLine1 = `Generated by: ${generatorName} | Date: ${todayStr}`;
      const headerLine2 = `Total SKUs: ${filteredStats.totalSkus} | Total Qty: ${filteredStats.totalQuantity}`;

      doc.text(headerLine1, 0.5, 1.4);
      doc.text(headerLine2, 0.5, 1.8);
      ...
      if (filterDesc) {
        doc.text(`Filters: ${filterDesc}`, 0.5, 2.2);
      }
