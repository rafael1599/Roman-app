...
> {
  ...
  constructor(
    ...
    optionsCallback?: (schemas: Record<string, z.ZodType<Row>>) => { schema: z.ZodType<Row> }
  ) {
   ...
   */
  protected validate(data: unknown): Row {
    ...
    if (this.schema) {
      try {
        ...
        if (err instanceof z.ZodError) {
          throw new AppError(
            `Data integrity violation in ${String(this.table)}`,
            'VALIDATION_ERROR',
            err
          );
        }
   ...
   */
  protected validateArray(data: unknown[] | null): Row[] {
    if (!data) return [];
