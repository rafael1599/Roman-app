 ...
 */
async function verifyWithGemini(imageFile: File, expectedItems: AIOrderItem[]): Promise<AIPalletVerification> {
    if (!genAI) {
        throw new Error('Gemini API not initialized');
    }
 ...
 */
async function verifyWithOpenAI(imageFile: File, expectedItems: AIOrderItem[]): Promise<AIPalletVerification> {
    if (!openai) {
        throw new Error('OpenAI API not initialized');
    }
...
export async function scanOrderImage(imageFile: File): Promise<AIOrderItem[]> {
    ...
    try {
        ...
        console.warn('⚠️ Tesseract scan failed:', error instanceof Error ? error.message : 'Unknown error');
    ...
    if (genAI) {
        try {
            ...
            console.warn('⚠️ Gemini scan failed:', error instanceof Error ? error.message : 'Unknown error');
            lastError = error instanceof Error ? error : new Error(String(error));
    ...
    if (openai) {
        try {
            ...
            console.error('❌ OpenAI scan also failed:', error instanceof Error ? error.message : 'Unknown error');
            throw new Error(`All OCR services failed.Last error: ${error instanceof Error ? error.message : 'Unknown error'} `);
        }
    }
 ...
 * Priority: Tesseract (free) -> Gemini -> OpenAI
 */
export async function verifyPalletImage(imageFile: File, expectedItems: AIOrderItem[]): Promise<AIPalletVerification> {
    let lastError: Error | null = null;
    ...
    try {
        ...
        console.warn('⚠️ Tesseract verification failed:', error instanceof Error ? error.message : 'Unknown error');
        lastError = error instanceof Error ? error : new Error(String(error));
        // Continue to AI fallbacks
    }
    ...
    if (genAI) {
        try {
            ...
            console.warn('⚠️ Gemini verification failed:', error instanceof Error ? error.message : 'Unknown error');
            lastError = error instanceof Error ? error : new Error(String(error));
    ...
    if (openai) {
        try {
            ...
            console.error('❌ OpenAI verification also failed:', error instanceof Error ? error.message : 'Unknown error');
            throw new Error(`All OCR services failed.Last error: ${error instanceof Error ? error.message : 'Unknown error'} `);
        }
    }
