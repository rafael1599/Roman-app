    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (skuMatch) {
            ...
            if (qty === 1 && i + 1 < lines.length) {
                ...
                for (const pattern of qtyPatterns) {
                    const qtyMatch = nextLine.match(pattern);
                    if (qtyMatch) {
                        qty = parseInt(qtyMatch[1]);
                        break;
                    }
 ...
 */
export async function scanOrderWithTesseract(file) {
    try {
 ...
 */
export async function verifyPalletWithTesseract(file, expectedItems) {
    try {
