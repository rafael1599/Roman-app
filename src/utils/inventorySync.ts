 ...
 */
export type PaginatedInventoryData = { data: InventoryItemWithMetadata[]; count: number };
type InventoryCacheData = InventoryItemWithMetadata[] | PaginatedInventoryData;

export function updateInventoryCache(
  oldData: InventoryCacheData | undefined | null,
  event: RealtimeInventoryEvent,
  filters: InventoryFilters,
  metadataMap?: Record<string, SKUMetadata>
): InventoryCacheData | undefined | null {
  ...
  switch (eventType) {
    case 'INSERT': {
      if (isMatch && newItem) {
        ...
        const localMatchIndex = nextItems.findIndex((i: InventoryItemWithMetadata) => {
          const isTempId =
            (typeof i.id === 'string' &&
              (String(i.id).startsWith('add-') || String(i.id).startsWith('move-'))) ||
        ...
        const enriched: InventoryItemWithMetadata = {
          ...newItem,
          sku_metadata:
            metadataMap?.[newItem.sku] || (newItem as InventoryItemWithMetadata).sku_metadata,
          _lastUpdateSource: (newItem as InventoryItemWithMetadata)._lastUpdateSource || 'remote',
        };

        if (localMatchIndex !== -1) {
          console.log(
            `[SYNC] De-duplicating: Replacing local/optimistic item ${nextItems[localMatchIndex].id} with real DB record ${newItem.id}`
          );
    ...
    case 'UPDATE': {
      ...
      const existingIndex = nextItems.findIndex(
        (i: InventoryItemWithMetadata) => String(i.id) === String(newItem.id)
      );
      ...
      if (alreadyInView) {
        if (!isMatch) {
          ...
          nextItems = nextItems.map((item, idx) => {
            if (idx === existingIndex) {
              const isIncomingRemote =
                !(newItem as InventoryItemWithMetadata)._lastUpdateSource ||
                (newItem as InventoryItemWithMetadata)._lastUpdateSource === 'remote';
              const isExistingLocal = item._lastUpdateSource === 'local';
              const localUpdateAge = item._lastLocalUpdateAt
                ? Date.now() - item._lastLocalUpdateAt
                : Infinity;
              ...
              if (isExistingLocal && isIncomingRemote && localUpdateAge < 4000) {
                if (newItem.quantity !== item.quantity) {
                  console.log(
                    `[SYNC] Ignoring stale remote update for ${item.sku}: Local=${item.quantity}, Remote=${newItem.quantity}`
                  );
                  return item;
                }
              }
              ...
              return {
                ...
                ...newItem,
                sku_metadata:
                  metadataMap?.[newItem.sku] ||
                  item.sku_metadata ||
                  (newItem as InventoryItemWithMetadata).sku_metadata,
                _lastUpdateSource:
                  isExistingLocal && isIncomingRemote && newItem.quantity === item.quantity
                    ? 'local'
                    : (newItem as InventoryItemWithMetadata)._lastUpdateSource || 'remote',
              };
            }
        ...
        if (localMatchIndex !== -1) {
          console.log(
            `[SYNC] De-duplicating UPDATE: Replacing local/optimistic item ${nextItems[localMatchIndex].id} with real DB record ${newItem.id}`
          );
          nextItems = nextItems.map((item, idx) => (idx === localMatchIndex ? enriched : item));
        } else {
          nextItems = [enriched, ...nextItems];
    ...
    case 'DELETE': {
      ...
      nextItems = nextItems.filter((i: InventoryItemWithMetadata) => String(i.id) !== targetId);
      break;
    }
  }
  ...
  if (isPaginatedStructure) {
    const paginated = oldData as PaginatedInventoryData;
    return {
      ...paginated,
      data: nextItems,
      count: (paginated.count ?? 0) + (nextItems.length - items.length),
    };
  }
