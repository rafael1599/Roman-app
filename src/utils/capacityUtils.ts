 ...
 */
export const calculateCapacityRatio = (current: number, max: number = DEFAULT_MAX_CAPACITY): number => {
 ...
 */
export const calculateConsolidationPriority = (current: number, max: number = DEFAULT_MAX_CAPACITY): number => {
 ...
 */
export const wouldExceedCapacity = (incomingQty: number, currentQty: number, max: number = DEFAULT_MAX_CAPACITY): boolean => {
...
export const calculateSkuVelocity = (sku: string, allLogs: InventoryLogSimple[], daysWindow: number = 30): number | null => {
    const now = new Date();
