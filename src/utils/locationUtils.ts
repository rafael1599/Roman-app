...
export const resolveLocationName = (
    ...
    if (isNumeric) {
        ...
        const existsInArray = locations.some(l => l.warehouse === warehouse && l.location === rowLocation);
 ...
 */
export const validateLocationPermission = (isNew: boolean, isAdmin: boolean, locationName: string): boolean => {
    if (isNew && !isAdmin) {
