...
export const predictLocation = (input, existingLocations = []) => {
    if (!input || typeof input !== 'string') return { matches: [], exactMatch: false, bestGuess: null };

    const cleanInput = input.trim();
    // Check for exact match first
    const exactMatch = existingLocations.find(loc => loc.toLowerCase() === cleanInput.toLowerCase());
    if (exactMatch) {
        return {
            matches: [exactMatch],
            exactMatch: true,
