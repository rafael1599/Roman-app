...
interface PalletVerificationProps {
  expectedItems: VerificationItem[];
  onVerified: (result: PalletVerificationResult) => void;
  onCancel: () => void;
  palletNumber: number;
}
 ...
 */
export default function PalletVerification({ expectedItems, onVerified, onCancel, palletNumber }: PalletVerificationProps) {
  const [isVerifying, setIsVerifying] = useState(false);
  const [preview, setPreview] = useState<string | null>(null);
  const [result, setResult] = useState<PalletVerificationResult | null>(null);
              ...
              {isVerifying && (
                <div className="absolute inset-0 bg-main/70 flex items-center justify-center rounded-lg">
                  <div className="text-center">
                    ...
                    <p className="text-muted text-sm mt-2">Analyzing pallet contents</p>
                  </div>
                </div>
              )}
            </div>
