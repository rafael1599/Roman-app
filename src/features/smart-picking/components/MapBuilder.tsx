...
interface LocationBlockProps {
  ...
  location: string;
  position: number;
  onEdit: (location: string) => void;
  onDelete: (location: string) => void;
  isCustom: boolean;
}

function LocationBlock({ id, location, position, onEdit, onDelete, isCustom }: LocationBlockProps) {
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-green-400 font-bold text-base truncate">{location}</div>
...
function LocationModal({ location, onSave, onCancel }: LocationModalProps) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-card border-2 border-accent rounded-xl p-6 max-w-md w-full">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-muted text-sm font-semibold mb-2">Location Name</label>
            <input
              type="text"
              value={name}
...
export default function MapBuilder() {
  ...
  const [warehouse, setWarehouse] = useState<'ludlow' | 'ats'>('ludlow');
  ...
  useEffect(() => {
    const inventory = warehouse === 'ludlow' ? ludlowData : atsData;
    const inventoryLocations = [
      ...new Set(inventory.map((item) => item.location).filter((loc) => loc && loc.trim() !== '')),
    ] as string[];
  ...
  }, [warehouse, ludlowData, atsData, customLocations]);
  ...
  const saveMap = () => {
    ...
    const mapData: Record<string, { position: number; x: number; y: number }> = {};
  ...
  const resetMap = () => {
    showConfirmation(
      ...
      () => {
        ...
        const inventoryLocations = [
          ...new Set(
            inventory.map((item) => item.location).filter((loc) => loc && loc.trim() !== '')
          ),
        ] as string[];
        const allLocations = [...new Set([...inventoryLocations, ...customLocations])].sort();
        setLocations(allLocations);
  ...
  const handleAddLocation = (name: string) => {
    if (!customLocations.includes(name) && !locations.includes(name)) {
      ...
      showError(
        'Location Exists',
        'This location name already exists. Please choose a different name.'
      );
    }
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-green-400">Warehouse Map Editor</h2>
      ...
      <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="text-blue-300 text-sm">
            <strong>How it works:</strong> The picking route is calculated based on the order of
            locations here. Items from the top location will be picked first, then the second, and
            so on. You can add custom locations that don't exist in your inventory yet.
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {locations.map((location, index) => (
            <div key={location} className="flex items-center gap-2">
              <span
                className={`text-sm font-mono ${customLocations.includes(location) ? 'text-blue-400' : 'text-green-500'
                  }`}
              >
                {index + 1}. {location}
              </span>
              {index < locations.length - 1 && <span className="text-green-500/40">→</span>}
            </div>
          ))}
        </div>
      </div>
