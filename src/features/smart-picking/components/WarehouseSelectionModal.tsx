...
interface WarehouseSelectionModalProps {
  items: ValidatedItem[];
  onConfirm: (selections: Record<string, 'ludlow' | 'ats'>) => void;
  onCancel: () => void;
  singleMode?: boolean;
}
...
export default function WarehouseSelectionModal({
  ...
  const [selections, setSelections] = useState<Record<string, 'ludlow' | 'ats'>>({});
  const { showError } = useError();

  // Initialize selections if items already have a warehouse pre-selected (for on-the-fly change)
  useEffect(() => {
    const initial: Record<string, 'ludlow' | 'ats'> = {};
    items.forEach((item) => {
      const itemKey = item.id || item.sku;
      if (item.warehouse) {
        initial[itemKey] = item.warehouse;
      }
    });
    setSelections(initial);
  }, [items]);

  const handleSelect = (itemKey: string, warehouse: 'ludlow' | 'ats') => {
    setSelections((prev) => ({
      ...prev,
      [itemKey]: warehouse,
    }));
  ...
  const handleConfirm = () => {
    ...
    if (!allSelected) {
      showError(
        ...
        'Please select a warehouse for all items before confirming.'
      );
      return;
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted text-xs sm:text-sm">
                {singleMode
                  ? 'Change location for this specific line'
              ...
              <div key={itemKey} className="bg-surface border border-subtle rounded-xl p-4">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-muted text-sm">Quantity: {item.qty} units</div>
                  </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <Warehouse
                          size={18}
                          className={
                            selections[itemKey] === 'ludlow' ? 'text-green-500' : 'text-muted'
                          }
                        />
                        <span
                          className={`font-bold ${selections[itemKey] === 'ludlow' ? 'text-green-600' : 'text-content'}`}
                        >
                          Ludlow
                        </span>
                      </div>
                      {selections[itemKey] === 'ludlow' && (
                    ...
                    {item.ludlow && (
                      <div className="space-y-1">
                        <div className="text-sm text-muted">
                          ...
                          <span
                            className={
                              selections[itemKey] === 'ludlow' ? 'text-green-600 font-bold' : ''
                            }
                          >
                            {item.ludlow.available}
                          </span>
                        </div>
                        <div className="text-sm text-muted">
