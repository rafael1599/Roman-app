...
interface SKUEditorModalProps {
  items: ValidatedItem[];
  inventory: any[];
  onConfirm: (corrections: Array<{ original: string; corrected: string; qty: number }>) => void;
  onCancel: () => void;
}
 ...
 */
export default function SKUEditorModal({ items, inventory, onConfirm, onCancel }: SKUEditorModalProps) {
  ...
  const skuSuggestions = useMemo(() => {
    const skuMap = new Map<string, { value: string; info: string }>();
      ...
      <div className="bg-card border-2 border-accent rounded-xl max-w-3xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-subtle flex-shrink-0">
          <div>
            ...
            <p className="text-muted text-sm mt-1">Correct the SKUs or select from suggestions</p>
          </div>
          <button onClick={onCancel} className="text-muted hover:text-content transition-colors">
            <X size={24} />
          </button>
        </div>
            ...
            <div key={index} className="bg-surface border border-subtle rounded-lg p-4">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-muted text-sm">Quantity: {item.qty}</div>
                </div>
                {item.isValid && <Check className="text-green-500" size={20} />}
              </div>

              {/* Manual Input with Autocomplete */}
              <div className="mb-3">
                <AutocompleteInput
                  id={`inventory_sku_${index}`}
                  value={item.edited}
                  onChange={(newValue: string) => updateSKU(index, newValue)}
              ...
              {item.suggestions.length > 0 && (
                <div>
                  <div className="text-muted text-xs mb-2">💡 Similar SKUs (click to select):</div>
