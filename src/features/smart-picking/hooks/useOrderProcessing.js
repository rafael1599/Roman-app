...
export function useOrderProcessing() {
  const { ludlowInventory, atsInventory, updateLudlowInventory, updateAtsInventory } =
    useInventory();
  ...
  const findInventoryItem = useCallback(
    (sku) => {
      ...
      if (ludlowNormalized) {
        console.log(
          `✅ Found SKU match: "${sku}" → "${ludlowNormalized.SKU}" (normalized, Ludlow)`
        );
  ...
  const validateOrder = useCallback(
    (orderItems, warehousePreferences = {}) => {
  ...
  const processOrder = useCallback(
    (scannedItems, warehousePreferences = {}, orderId = null) => {
