...
export const usePickingListsSubscription = () => {
  ...
  useEffect(() => {
    let retryTimeout: ReturnType<typeof setTimeout> | undefined = undefined;

    const setupSubscription = () => {
      console.log(
        '[FORENSIC][REALTIME][PICKING_LISTS_INIT]',
        new Date().toISOString(),
        `- Setting up channel (Attempt ${retryCountRef.current + 1}/${MAX_RETRIES})`
      );
      ...
      const channel = supabase
        ...
        .subscribe((status) => {
          console.log(
            '[FORENSIC][REALTIME][PICKING_LISTS_STATUS]',
            new Date().toISOString(),
            '- Status:',
            status
          );
          ...
          if (status === 'SUBSCRIBED') {
            ...
            console.log(
              '[FORENSIC][REALTIME][PICKING_LISTS_SUBSCRIBED] ✅ Successfully subscribed'
            );
          }
          ...
          if (status === 'CLOSED' || status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
            if (retryCountRef.current < MAX_RETRIES) {
              ...
              console.error(
                '[FORENSIC][REALTIME][PICKING_LISTS_FAILED] Max retries reached. Subscription failed.'
              );
              toast.error('Picking live updates disconnected. Pull down to refresh.', {
                id: 'realtime-picking-error',
              });
            }
          }
        });
