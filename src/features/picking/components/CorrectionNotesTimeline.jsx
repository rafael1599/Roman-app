        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between px-1 mb-2">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-accent">Instructions</h3>
                </div>
                {hasHistory && (
                    <button
                    ...
                    >
                        ...
                        {isExpanded ? <ChevronUp size={10} className="text-muted" /> : <ChevronDown size={10} className="text-muted" />}
                    </button>
                )}
            </div>
            ...
            <div className="relative pl-4 border-l-2 border-subtle/30 space-y-4">
                ...
                {isExpanded && historicalNotes.map((note) => (
                    <NoteItem key={note.id} note={note} isHistorical />
                ))}
...
const NoteItem = ({ note, isLatest, isHistorical }) => (
    <div className={`relative ${isHistorical ? 'opacity-60 hover:opacity-100 transition-opacity' : ''}`}>
        {/* Dot on time line */}
        <div className={`absolute -left-[1.35rem] top-1.5 w-2 h-2 rounded-full border-4 border-card ${isLatest ? 'bg-accent' : 'bg-subtle'}`} />

        <div className={`${isLatest ? 'bg-surface border-accent/20' : 'bg-surface/30 border-subtle/30'} border rounded-2xl p-4 shadow-sm animate-in fade-in slide-in-from-left-2 duration-300`}>
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center ${isLatest ? 'bg-accent/10 text-accent' : 'bg-muted/10 text-muted'}`}>
                        <User size={10} />
                    </div>
            ...
            <p className={`text-sm leading-relaxed font-medium bg-card/40 p-3 rounded-xl border border-subtle/20 italic ${isLatest ? 'text-content' : 'text-content/70'}`}>
                "{note.message}"
            </p>
