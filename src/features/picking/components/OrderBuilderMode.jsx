...
export const OrderBuilderMode = ({ cartItems, onGeneratePath }) => {
    ...
    const handleQuantityClick = (index, currentQty) => {
        ...
        <div className="flex flex-col h-full bg-card">
            ...
            <div className="px-4 py-3 bg-surface border-b border-subtle">
                <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-muted">Add all items you need, then generate the picking path.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {cartItems.map((item, index) => (
                    <div key={`${item.SKU}-${index}`} className="flex items-center justify-between bg-surface p-3 rounded-lg border border-subtle shadow-sm animate-in fade-in slide-in-from-bottom-2">
                        <div className="flex-1 mr-4">
                            ...
                            <div className="flex items-center gap-2 pl-9">
                                {item.Warehouse && <span className="text-[10px] border px-1 rounded text-slate-400">{item.Warehouse}</span>}
                                {item.notes && <span className="bg-yellow-50 text-yellow-700 px-1 rounded border border-yellow-100 text-[10px] truncate max-w-[120px]">{item.notes}</span>}
                            </div>
                        </div>
