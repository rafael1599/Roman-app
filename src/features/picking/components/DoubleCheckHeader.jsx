            ...
            {isOpen && (
                <div className="absolute right-0 mt-3 w-72 bg-card border border-subtle rounded-2xl shadow-2xl z-[100] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="p-4 border-b border-subtle bg-surface/50">
                        <h3 className="text-xs font-black text-content uppercase tracking-widest">Verification Queue</h3>
                    </div>
                    ...
                    <div className="max-h-96 overflow-y-auto">
                        ...
                        {correctionCount > 0 && (
                            <div className="p-2">
                                <p className="px-2 py-1 text-[9px] font-black text-amber-500/70 uppercase tracking-widest">Clarification</p>
                                {orders.filter(o => o.status === 'needs_correction').map(order => (
                                    <div key={order.id} className="flex items-center gap-1 pr-2 rounded-xl hover:bg-amber-500/5 transition-colors group">
                                        <button
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="text-xs font-black text-content uppercase">#{order.order_number || order.id.slice(-6).toUpperCase()}</div>
                                                    <div className="text-[9px] text-muted font-bold uppercase">{order.profiles?.full_name}</div>
                                                </div>
                                            </div>
                                            <ChevronDown size={14} className="-rotate-90 text-subtle group-hover:text-amber-500 transition-colors" />
                                        </button>
                                        {isAdmin && (
                                            <button
                                                onClick={(e) => {
                        ...
                        <div className="p-2 border-t border-subtle">
                            <p className="px-2 py-1 text-[9px] font-black text-accent uppercase tracking-widest">Ready to Verify</p>
                            {orders.filter(o => o.status === 'ready_to_double_check' || o.status === 'double_checking').map(order => (
                                <div key={order.id} className="flex items-center gap-1 pr-2 rounded-xl hover:bg-accent/5 transition-colors group">
                                    <button
                                        onClick={() => handleOrderSelect(order)}
                                        className={`flex-1 flex items-center justify-between p-3 text-left ${order.status === 'double_checking' ? 'opacity-60' : ''}`}
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-colors ${order.status === 'double_checking'
                                                ? 'bg-orange-500/10 text-orange-500 border-orange-500/20'
                                                : 'bg-accent/10 text-accent border-accent/20'
                                                }`}>
                                                {order.status === 'double_checking' ? <Clock size={16} /> : <CheckCircle2 size={16} />}
                                            </div>
                                            <div>
                                                <div className="text-xs font-black text-content uppercase">#{order.order_number || order.id.slice(-6).toUpperCase()}</div>
                                        ...
                                        <ChevronDown size={14} className="-rotate-90 text-subtle group-hover:text-accent transition-colors" />
                                    </button>
                                    {isAdmin && (
                                        <button
                                            onClick={(e) => {
                            ...
                            {readyCount === 0 && orders.filter(o => o.status === 'double_checking').length === 0 && (
