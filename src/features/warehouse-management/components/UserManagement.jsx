...
import { UserPlus, UserRoundPen, Trash2, Mail, Shield, User, Loader2, Key, Save, ShieldCheck, ShieldOff } from 'lucide-react';
...
export const UserManagement = () => {
    ...
    const handleDelete = async (userId) => {
        // Kept for backward compatibility or if we really need to delete
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center bg-card border border-subtle p-4 rounded-2xl">
                <div>
                    <h3 className="text-sm font-black uppercase tracking-widest text-content">System Users</h3>
                    <p className="text-[10px] text-muted font-bold uppercase mt-1">Manage staff access and permissions</p>
                </div>
                <button
                    onClick={() => {
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                    ...
                    <p className="text-xs text-muted font-bold uppercase tracking-widest">Loading user accounts...</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {users.map((u) => (
                        <div key={u.id} className="bg-card border border-subtle p-5 rounded-2xl hover:border-accent/30 transition-all group relative">
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    <div className={`p-3 rounded-2xl ${!u.is_active ? 'bg-red-500/10 text-red-500' : u.role === 'admin' ? 'bg-amber-500/10 text-amber-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
                                        <Shield size={20} />
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <h4 className={`font-bold text-content leading-tight uppercase tracking-tight ${!u.is_active ? 'opacity-40' : ''}`}>
                                                {u.full_name}
                                            </h4>
                                            {!u.is_active && (
                                                <span className="text-[8px] font-black bg-red-500 text-white px-1.5 py-0.5 rounded uppercase">Inactive</span>
                                            )}
                                        </div>
                                        <span className={`text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded border mt-1 inline-block ${u.role === 'admin' ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500'}`}>
                                            {u.role}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-2 mt-4">
                                <div className="flex items-center gap-2 text-muted">
                                    ...
                                    <span className="text-xs font-medium truncate opacity-60">{u.email || 'No email set'}</span>
                                </div>
                                <div className="flex items-center gap-2 text-muted">
                                    <User size={14} className="opacity-40" />
                                    <span className="text-[10px] font-mono opacity-40 uppercase truncate">ID: ...{u.id.slice(-8)}</span>
                                </div>
                                <div className="flex items-center gap-2 text-muted mt-1">
                                    <div className={`w-1.5 h-1.5 rounded-full ${u.last_seen_at && (new Date() - new Date(u.last_seen_at)) < 300000 ? 'bg-emerald-500 animate-pulse' : 'bg-subtle'}`} />
            ...
            <div className="relative w-full max-w-md bg-card border border-subtle rounded-[2rem] overflow-hidden shadow-2xl animate-in zoom-in slide-in-from-bottom-8 duration-500">
                <div className="p-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted ml-1">System Role</label>
                        ...
                        <div className="space-y-4">
                            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted ml-1">Account Controls</label>

                            <div className="flex items-center justify-between p-4 bg-main border border-subtle rounded-2xl">
                                <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-lg ${formData.is_active ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[9px] text-muted font-medium uppercase">{formData.is_active ? 'Account is operational' : 'Account is disabled'}</p>
                                    </div>
                                </div>
                                <button
                                    type="button"
