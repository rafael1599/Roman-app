      ...
      <div className="bg-surface border border-subtle rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden scale-100 animate-in zoom-in-95 duration-200">
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          <form onSubmit={handleSubmit(onFormSubmit)} className="p-6 space-y-6">
            ...
            {validationState.status !== 'idle' && (
              <div
                className={`mt-2 flex items-start gap-2 p-3 rounded-xl text-[10px] font-black uppercase tracking-widest animate-in fade-in slide-in-from-top-1 ${
            ...
            <div>
              <label
                htmlFor="inventory_internal_note"
                className="block text-[10px] font-black text-accent mb-2 uppercase tracking-widest"
              >
                  ...
                  {distribution.map((row, idx) => (
                    ...
                    >
                      ...
                      <input
                        ...
                        onBlur={(e) => {
                          if (e.target.value === '' || Number(e.target.value) < 1)
                            updateDistributionRow(idx, 'count', 1);
                        }}
                        {...autoSelect}
                      ...
                      <input
                        ...
                        onBlur={(e) => {
                          if (e.target.value === '' || Number(e.target.value) < 1)
                            updateDistributionRow(idx, 'units_each', 1);
                        }}
                        {...autoSelect}
                        className="w-14 bg-surface border border-subtle rounded-lg px-2 py-2 text-content text-center text-xs font-mono font-bold focus:border-accent focus:outline-none"
                        min={1}
            ...
            {isAdmin && (
              <div className="grid grid-cols-4 gap-3 p-4 bg-accent/5 rounded-2xl border border-accent/10">
                <div>
                  ...
                  <input
                    type="number"
                    {...register('length_in', { valueAsNumber: true })}
                    {...autoSelect}
                    step="0.1"
                    className="w-full bg-main border border-subtle rounded-lg px-2 py-2 text-content focus:border-accent focus:outline-none font-mono text-center text-xs"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-accent mb-2 uppercase tracking-widest">
                  ...
                  <input
                    type="number"
                    {...register('width_in', { valueAsNumber: true })}
                    {...autoSelect}
                    step="0.1"
                    className="w-full bg-main border border-subtle rounded-lg px-2 py-2 text-content focus:border-accent focus:outline-none font-mono text-center text-xs"
                  />
        ...
        <div className="p-6 border-t border-subtle bg-main/50 flex gap-3">
          ...
          <button
            ...
            className={`flex-1 font-black uppercase tracking-widest h-14 rounded-2xl flex items-center justify-center gap-2 transition-transform shadow-lg shadow-accent/20 ${
              !isValid ||
              !sku?.trim() ||
              !location?.trim() ||
              validationState.status === 'error' ||
