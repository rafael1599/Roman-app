...
import { usePickingListsSubscription } from '../../picking/hooks/usePickingListsSubscription';
...
interface InventoryContextType {
  ...
  ludlowInventory: InventoryItemWithMetadata[];
  atsInventory: InventoryItemWithMetadata[];
  ...
  updateItem: (originalItem: InventoryItemWithMetadata, updatedFormData: InventoryItemInput) => Promise<void>;
  moveItem: (
  ...
  updateInventory: (newData: InventoryItemWithMetadata[]) => void;
  updateLudlowInventory: (newData: InventoryItemWithMetadata[]) => void;
  updateAtsInventory: (newData: InventoryItemWithMetadata[]) => void;
...
export const InventoryProvider = ({
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      try {
        ...
        const enriched: InventoryItemWithMetadata[] = (data || [])
          .filter((item: any) => item.warehouse !== 'ATS')
          .map((item: any) => {
  ...
  const updateItem = useCallback(
    async (originalItem: InventoryItemWithMetadata, updatedFormData: InventoryItemInput & { isReversal?: boolean }) => {
      try {
  ...
  const updateInventory = useCallback((newData: InventoryItemWithMetadata[]) => {
    setInventoryData(newData);
  }, []);

  const updateLudlowInventory = useCallback(
    (newData: InventoryItemWithMetadata[]) => {
      setInventoryData((prev) => [
