...
describe('InventoryService', () => {
  describe('checkExistence', () => {
    it('should return true if item exists at coordinates', async () => {
      ...
      const exists = await inventoryService.checkExistence('SKU-TEST', 'Row 1', 'LUDLOW');
    ...
    it('should return false if item does not exist', async () => {
      ...
      const exists = await inventoryService.checkExistence('SKU-NONE', 'Row 2', 'ATS');
  ...
  describe('updateItem', () => {
    const mockCtx: InventoryServiceContext = {
      isAdmin: true,
    ...
    it('should throw Conflict error if renaming to an existing SKU in target location', async () => {
      ...
      await expect(
        inventoryService.updateItem(originalItem, updatedData, mockLocations, mockCtx)
