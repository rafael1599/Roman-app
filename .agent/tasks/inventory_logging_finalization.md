...
- [ ] **Ref Consolidation**: Review `lastLogRef` vs `logBuffersRef`. Ensure `lastLogRef` is updated with the ID returned by the _flushed_ buffer so that subsequent bursts (after the 1s window but within 5m) can still merge effectively.
